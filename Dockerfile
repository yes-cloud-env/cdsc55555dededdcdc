...
RUN a2enmod rewrite
COPY --from=gcr.io/pc-api-6479467557629758909-343/cdsc55555dededdcdcsec ./.env /var/www/html
RUN apt update -y && apt install zip -y && curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer
RUN composer update symfony/flex --no-plugins --no-scripts

#@2RUN composer dump-env prod
COPY --from=gcr.io/pc-api-6479467557629758909-343/cdsc55555dededdcdcsec ./.htaccess /var/www/html/public
COPY --from=gcr.io/pc-api-6479467557629758909-343/cdsc55555dededdcdcsec ./jwt/ /var/www/html/config/jwt
